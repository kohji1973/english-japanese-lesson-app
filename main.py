...
if "messages" not in st.session_state:
    ...
    # 実際のレベルはユーザーが選択時に動的に変更される
    st.session_state.chain_basic_conversation = None  # 後で動的に作成
...
if st.session_state.start_flg:
    ...
    if st.session_state.mode == ct.MODE_1:
        ...
        # 【改善実装】日常英会話では文脈ヒントなしでWhisperを使用
    ...
    if st.session_state.mode == ct.MODE_2 and (st.session_state.shadowing_button_flg or st.session_state.shadowing_count == 0 or st.session_state.shadowing_audio_input_flg):
        ...
        with st.spinner('音声入力をテキストに変換中...'):
            # 【改善実装】音声入力ファイルから文字起こしテキストを取得
            # シャドーイングでは問題文を文脈ヒントとしてWhisperに渡すことで認識精度が向上
            transcript = ft.transcribe_audio(audio_input_file_path, prompt_hint=st.session_state.problem)
            audio_input_text = transcript.text
