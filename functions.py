...
def transcribe_audio(audio_input_file_path, prompt_hint=None):
    """
    【改善4】Whisperの精度向上：文脈プロンプトとtemperature調整
    
    音声入力ファイルから文字起こしテキストを取得
    
    Args:
        audio_input_file_path: 音声入力ファイルのパス
        prompt_hint: Whisperに与える文脈ヒント（オプション）
                     問題文がある場合はそれを渡すと精度が向上
    
    精度向上のポイント：
    - prompt: 文脈や専門用語を事前に与えることで認識精度が向上
    - temperature: 0.0に設定することで、より一貫性のある結果を得られる
    - language: 明示的に"en"を指定して英語として認識させる
    """

    with open(audio_input_file_path, 'rb') as audio_input_file:
        # promptパラメータで文脈を与えると、Whisperの認識精度が向上
        # 特にシャドーイングやディクテーションで元の文章を与えると効果的
        if prompt_hint:
            transcript = st.session_state.openai_obj.audio.transcriptions.create(
                model="whisper-1",
                file=audio_input_file,
                language="en",
                prompt=prompt_hint,  # 文脈ヒントを与える
